...
CREATE TABLE Vehicle(
	...
	stop INT,
    CONSTRAINT FK_VEHICLE_LINE FOREIGN KEY (line) REFERENCES Line(lineNumber) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE Identifier (
    ...
    type INT,
    passenger INT,
    CONSTRAINT FK_IDENTIFIER_PASSENGER FOREIGN KEY (passenger) REFERENCES Passenger(ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_IDENTIFIER_CARD_TYPE FOREIGN KEY (type) REFERENCES IdentifierType(ID) ON DELETE CASCADE ON UPDATE CASCADE,
);

CREATE TABLE Validate(
	vehicle VARCHAR(20) NOT NULL REFERENCES Vehicle(registrationNumber) ON DELETE CASCADE ON UPDATE CASCADE,
	identifier INT NOT NULL REFERENCES Identifier(ID) ON DELETE CASCADE ON UPDATE CASCADE,
	validation DATETIME,
